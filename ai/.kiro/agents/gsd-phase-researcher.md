...
**.kiro/steering/ enforcement:** If `.kiro/steering/` exists, extract all actionable directives (required tools, forbidden patterns, coding conventions, testing rules, security requirements). Include a `## Project Constraints (from .kiro/steering/)` section in RESEARCH.md listing these directives so the planner can verify compliance. Treat .kiro/steering/ directives with the same authority as locked decisions from CONTEXT.md — research should not recommend approaches that contradict them.
</project_context>
...
```bash
node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
```bash
INIT=$(node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
**ALWAYS use the Write tool to create files** — never use `execute_bash(cat << 'EOF')` or heredoc commands for file creation. Mandatory regardless of `commit_docs` setting.
...
```bash
node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
