...
## Dimension 10: .kiro/steering/ Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from .kiro/steering/?

**Process:**
1. Read `.kiro/steering/` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns .kiro/steering/ explicitly forbids
5. Flag plans that skip steps .kiro/steering/ explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern .kiro/steering/ explicitly forbids
- Plan skips a required step (e.g., .kiro/steering/ says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts .kiro/steering/ conventions
- Plan creates files in locations that violate .kiro/steering/'s architectural constraints
- Plan ignores security requirements documented in .kiro/steering/

**Skip condition:** If no `.kiro/steering/` exists in the working directory, output: "Dimension 10: SKIPPED (no .kiro/steering/ found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but .kiro/steering/ requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project .kiro/steering/"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by .kiro/steering/"
  plan: "02"
...
```bash
INIT=$(node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
- [ ] .kiro/steering/ compliance checked (plans respect project conventions)
