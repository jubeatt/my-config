...
Use template from `$HOME/.kiro/get-shit-done/templates/UI-SPEC.md`.
...
Read template: `$HOME/.kiro/get-shit-done/templates/UI-SPEC.md`
...
```bash
node "$HOME/.kiro/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): UI design contract" --files "$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md"
```
